...
{
  int num;
  if (get_cell(i,j) ==color || get_cell(i,j) == player)
    {
      if (!(board_visited[i*BOARD_SIZE + j]))
	{
	  ...
	  if (is_inb(i+1,j))
	    {
	      num += (calc_new_cells_rec(player, color, i+1, j));
	    }
	  if (is_inb (i-1, j))
	    {
	      num += (calc_new_cells_rec(player, color, i-1 ,j));
	  ...
	  if (is_inb (i, j+1))
	    {
	      num += (calc_new_cells_rec(player, color, i, j+1));
	    }
	  if (is_inb (i, j-1))
	    {
	      num += (calc_new_cells_rec(player, color, i, j-1));
...
char IA_greedy()
{
	...
	{
		temp_cells = calc_new_cells_optimized('v', color);
